...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    let [rows] = await connection.execute(
      "SELECT id, username, full_name, phone, email, password, is_blocked FROM admins WHERE email = ?",
      [email]
    );
    ...
    if (rows.length > 0) {
      ...
      // ✅ BLOCK CHECK (admins.is_blocked = 1 means blocked)
    ...
    [rows] = await connection.execute(
      "SELECT id, username, full_name, phone, email, password FROM super_admin WHERE email = ?",
      [email]
    );
