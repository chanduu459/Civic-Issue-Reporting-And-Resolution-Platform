...
async function findPerformerByEmail(email) {
  ...
  try {
    ...
    if (rows.length > 0) {
      connection.release();
      return { table: "super_admin", id: rows[0].id };
    }
 ...
 * @param {String} deletedFromTable - Table name where deletion occurred (e.g., "admins", "officers")
 * @param {String} performerTable - Table where performer belongs (e.g., "super_admin", "admins", "officers")
 */
