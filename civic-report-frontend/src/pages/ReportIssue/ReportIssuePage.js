...
function ReportIssuePage() {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (data.success) {
        const matchedName =
          typeof data.matchedName === "string" ? data.matchedName.trim() : "";

        setOtpVerified(true);
        if (matchedName) {
          setFullName(matchedName);
          setNameAutofilled(true);
          setOtpVerifyMsg("✅ Successfully verified. Name auto-filled.");
        } else {
          setNameAutofilled(false);
          setOtpVerifyMsg("✅ Successfully verified. Please enter your full name.");
        }
      } else {
        setOtpVerified(false);
        setNameAutofilled(false);
